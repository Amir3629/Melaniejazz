...
export default function SubmitButton({
  ...
  return (
    ...
    >
      {isSubmitting ? (
        <Loader2 className="hidden sm:block w-5 h-5 mr-2 animate-spin" />
      ) : isLastStep ? (
        <Check className="hidden sm:block w-5 h-5 mr-2" />
      ) : (
        <ArrowRight className="hidden sm:block w-5 h-5 mr-2" />
      )}
